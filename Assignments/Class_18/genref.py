...
def genRef(method):
    ...
    refs_str = input(
        "Enter times and angles, starting at t=0 (ex: 0 0 1 90 2 90 3 0): "
    )
    ...
    if method == "cubic":
        ...
        if numpos >= 3:
            for i in range(1, numpos - 1):
                vel_list[i] = (pos_list[i + 1] - pos_list[i - 1]) / (
        ...
        for i in range(0, numpos - 1):
            ...
            a1 = velstart
            a2 = (
                3 * posend - 3 * posstart - 2 * velstart * deltaT - velend * deltaT
            ) / (deltaT * deltaT)
            a3 = (2 * posstart + (velstart + velend) * deltaT - 2 * posend) / (
