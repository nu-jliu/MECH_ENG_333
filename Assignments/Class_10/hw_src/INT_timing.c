...
void __ISR(_EXTERNAL_0_VECTOR, IPL6SOFT) Ext0ISR(void)
{
    ...
    NU32DIP_YELLOW = !NU32DIP_YELLOW; // turn off LED2
...
void __ISR(_EXTERNAL_1_VECTOR, IPL6SOFT) Ext1ISR(void)
{
    ...
    NU32DIP_YELLOW = !NU32DIP_YELLOW; // turn on LED2
    Exited = _CP0_GET_COUNT();        // record time ISR ends
}
...
int main(void)
{
    ...
    INTCONbits.INT0EP = 1;          // step 3: INT0 trigger on rising edge
    INTCONbits.INT1EP = 1;          // step 3: INT1 trigger on rising edge
    IPC0bits.INT0IP = 0b110;        // step 4: Set priority bits for INT0
    IPC0bits.INT0IS = 0b00;         // step 4: Set subpriority bits for INT0
    // IPC0 |= 24 << 24;               // step 4: set INT0 to priority 6 subpriority 0
    // IPC1CLR = 0x1F << 24;          // step 4: clear 5 priority and subp bits for INT1
    IPC1bits.INT1IP = 0b110;       // step 4: set INT1 to priority 6
    IPC1bits.INT1IS = 0b00;        // step 4: set INT1 to subpriority 0
    IFS0bits.INT0IF = 0;           // step 5: clear INT0 flag status
    IFS0bits.INT1IF = 0;           // step 5: clear INT1 flag status
    IEC0bits.INT0IE = 1;           // step 6: enable INT0 interrupts
    IEC0bits.INT1IE = 1;           // step 6: enable INT1 interrupts
